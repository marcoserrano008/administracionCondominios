...
const Ingresos = () => {
  ...
  const handleOnSubmit = async (event) => {
    ...
    // Verifica que el formulario tiene todos los datos necesarios.
    if (!form.edificio) {
    ...
    const submitData = {
      ...
      registroSalida: 'no', // Asumiendo que 'no' indica que aún no ha salido.
      idIngreso: generateIdIngreso(getGTMMinus4Date()),
    };
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/api/ingresos', {
        ...
        headers: {
          ...
          // Aquí incluye cualquier otro encabezado que necesite tu API, como tokens de autenticación.
        },
      ...
      // Si necesitas el ID de respuesta, puedes obtenerlo así:
      const result = await response.json();
      console.log("Document written with ID: ", result.id); // Asumiendo que tu API devuelve un objeto con un 'id'.
      setIdTicket(submitData.idIngreso);
